...
export async function getTweetRetweeters(
  ...
  const usersPaginated = await world.twitter.v2.tweetRetweetedBy(tweetId, {
    asPaginator: true,
  ...
  while (!usersPaginated.done) {
    if (usersPaginated.rateLimit.remaining === 0) {
      const timeToWait =
        usersPaginated.rateLimit.reset - Date.now() / 1000 + 10;
      console.log(
        'waiting due to rate limit. next rate reset timestamp:',
        usersPaginated.rateLimit.reset
      );
